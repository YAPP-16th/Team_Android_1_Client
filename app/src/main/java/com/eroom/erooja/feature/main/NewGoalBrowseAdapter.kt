...
private val context: Context) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        ...
        return ViewHolder(mBinding, mBinding.title, mBinding.jobClass, mBinding.term, mBinding.itemAll, mBinding.imageFrame)
    }
        ...
        when (items[position].userImages.size) {
            3 -> {
                if (items[position].joinCount > 3) {
                    holder.frame.addView(ItemCircleProfile(context, items[position].userImages[2], 60f))
                    holder.frame.addView(ItemCircleProfile(context, items[position].userImages[1], 40f))
                    holder.frame.addView(ItemCircleProfile(context, items[position].userImages[0], 20f))
                    holder.frame.addView(ItemCircleJoinCount(context, items[position].joinCount - 3))
                } else {
                    holder.frame.addView(ItemCircleProfile(context, items[position].userImages[2], 40f))
                    holder.frame.addView(ItemCircleProfile(context, items[position].userImages[1], 20f))
                    holder.frame.addView(ItemCircleProfile(context, items[position].userImages[0], 0f))
                }
            }
            2 -> {
                if (items[position].joinCount > 2) {
                    holder.frame.addView(ItemCircleProfile(context, items[position].userImages[1], 40f))
                    holder.frame.addView(ItemCircleProfile(context, items[position].userImages[0], 20f))
                    holder.frame.addView(ItemCircleJoinCount(context, items[position].joinCount - 2))
                } else {
                    holder.frame.addView(ItemCircleProfile(context, items[position].userImages[1], 20f))
                    holder.frame.addView(ItemCircleProfile(context, items[position].userImages[0], 0f))
                }
            ...
            1 -> {
                if (items[position].joinCount > 1) {
                    holder.frame.addView(ItemCircleProfile(context, items[position].userImages[0], 20f))
                    holder.frame.addView(ItemCircleJoinCount(context, items[position].joinCount - 1))
                } else {
                    holder.frame.addView(ItemCircleProfile(context, items[position].userImages[0], 0f))
                }
            }
