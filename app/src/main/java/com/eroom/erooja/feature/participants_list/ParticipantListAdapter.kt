...
import com.eroom.data.entity.JobInterest
...
class ParticipantListAdapter(callback : DiffUtil.ItemCallback<Member>, private val list: ArrayList<Member>) : ListAdapter<Member, ParticipantViewHolder>(callback) {
    ...
    override fun onBindViewHolder(holder: ParticipantViewHolder, position: Int) {
        holder.name.text = list[position].nickname
        holder.jobClassText.text = list[position].jobInterests[0].name add if (list[position].jobInterests.size < 2) "" else "외 ${list[position].jobInterests.size - 1}"
