...
import com.eroom.erooja.app.feature.main.ItemCircleJoinCount
import com.eroom.erooja.app.feature.main.ItemCircleProfile
import kotlinx.android.synthetic.main.item_main_new_goal.view.*

class SearchResultAdapter(
    callback: DiffUtil.ItemCallback<GoalContent>,
    private val goalContent: ArrayList<GoalContent>,
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        holder.bind(
            goalContent[position].title,
            goalContent[position].joinCount,
            goalContent[position].startDt,
            goalContent[position].endDt,
            goalContent[position].isDateFixed,
            click,
            goalContent[position].id,
            goalContent[position].jobInterests
        )
            ...
            3 -> {
                if (goalContent[position].joinCount > 3) {
                    holder.frame.addView(
                        ItemCircleProfile(
                            context,
                            goalContent[position].userImages[2],
                            60f
                        )
                    )
                    holder.frame.addView(
                        ItemCircleProfile(
                            context,
                            goalContent[position].userImages[1],
                            40f
                        )
                    )
                    holder.frame.addView(
                        ItemCircleProfile(
                            context,
                            goalContent[position].userImages[0],
                            20f
                        )
                    )
                    holder.frame.addView(
                        ItemCircleJoinCount(
                            context,
                            goalContent[position].joinCount - 3
                        )
                    )
                } else {
                    holder.frame.addView(
            ...
            2 -> {
                if (goalContent[position].joinCount > 2) {
                    ...
                    holder.frame.addView(
                        ItemCircleJoinCount(
                            context,
                            goalContent[position].joinCount - 2
                        )
                    )
                } else {
                    holder.frame.addView(
            ...
            1 -> {
                if (goalContent[position].joinCount > 1) {
                    ...
                    holder.frame.addView(
                        ItemCircleJoinCount(
                            context,
                            goalContent[position].joinCount - 1
                        )
                    )
                } else {
                    holder.frame.addView(
...
class Holder(itemView: View, val frame: FrameLayout) : RecyclerView.ViewHolder(itemView) {
    fun bind(
        ...
        click: (Long) -> Unit,
        goalId: Long,
        jobInterests: ArrayList<GoalType>
    ) {
        itemView.title.text = title
        if (jobInterests.size > 0) {
            val extraJobClassSize = jobInterests.size - 1
            itemView.jobClass.text =
                if (extraJobClassSize == 0) jobInterests[0].name else "${jobInterests[0].name} 외 $extraJobClassSize"
        }
        itemView.term.text =
            if (fix) {
